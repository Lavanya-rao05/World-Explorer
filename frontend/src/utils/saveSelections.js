...
export const saveSelections = async () => {
  ...
  const restaurants =
    JSON.parse(localStorage.getItem("selectedRestaurants")) || [];
  ...
  const normalizedRestaurants = restaurants.map((restaurant) => ({
    ...
    latitude: restaurant.location?.lat || restaurant.lat || restaurant.latitude,
    longitude:
      restaurant.location?.lng || restaurant.lng || restaurant.longitude,
    category: "restaurant",
  }));
